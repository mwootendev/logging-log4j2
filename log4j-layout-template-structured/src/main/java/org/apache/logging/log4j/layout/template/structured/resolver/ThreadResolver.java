...
public final class ThreadResolver implements EventResolver {
    ...
    private static final EventResolver NAME_RESOLVER = (final LogEvent logEvent, final StructuredWriter jsonWriter) -> {
        final String threadName = logEvent.getThreadName();
        jsonWriter.writeString(threadName);
    };

    private static final EventResolver ID_RESOLVER = (final LogEvent logEvent, final StructuredWriter jsonWriter) -> {
        final long threadId = logEvent.getThreadId();
        jsonWriter.writeNumber(threadId);
    };

    private static final EventResolver PRIORITY_RESOLVER = (final LogEvent logEvent, final StructuredWriter jsonWriter) -> {

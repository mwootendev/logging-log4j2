...
public final class LevelResolver implements EventResolver {
    ...
    private static EventResolver createNameResolver(final StructuredWriter contextStructuredWriter) {
        final Map<Level, String> resolutionByLevel = Arrays.stream(Level.values())
                .collect(Collectors.toMap(
                        Function.identity(),
                        (final Level level) -> contextStructuredWriter.use(() -> {
                            final String name = level.name();
                            contextStructuredWriter.writeString(name);
                        })));
    ...
    private static EventResolver createSeverityKeywordResolver(final StructuredWriter contextStructuredWriter) {
        final Map<Level, String> resolutionByLevel = Arrays.stream(Level.values())
                .collect(Collectors.toMap(
                        Function.identity(),
                        (final Level level) -> contextStructuredWriter.use(() -> {
                            ...
                            contextStructuredWriter.writeString(severityKeyword);
                        })));
        return (final LogEvent logEvent, final StructuredWriter jsonWriter) -> {
            final Level level = logEvent.getLevel();

 ...
 * TemplateLayoutDefaults.Builder#setLocationInfoEnabled(boolean) locationInfoEnabled}
...
public final class SourceResolver implements EventResolver {
    ...
    private static final EventResolver NULL_RESOLVER =
            (final LogEvent value, final StructuredWriter jsonWriter) -> jsonWriter.writeNull();

    private static final EventResolver CLASS_NAME_RESOLVER = (final LogEvent logEvent, final StructuredWriter jsonWriter) -> {
    ...
    private static final EventResolver FILE_NAME_RESOLVER = (final LogEvent logEvent, final StructuredWriter jsonWriter) -> {
        final StackTraceElement logEventSource = logEvent.getSource();
        if (logEventSource == null) {
            jsonWriter.writeNull();
        } else {
    ...
    private static final EventResolver LINE_NUMBER_RESOLVER =
            (final LogEvent logEvent, final StructuredWriter jsonWriter) -> {
                final StackTraceElement logEventSource = logEvent.getSource();
                if (logEventSource == null) {
                    jsonWriter.writeNull();
                } else {

...
public class CounterResolver implements EventResolver {
    ...
    private final Consumer<StructuredWriter> delegate;
    ...
    private static Consumer<StructuredWriter> createDelegate(
    ...
    private static Consumer<StructuredWriter> createLongResolver(final BigInteger start) {
    ...
    private static Consumer<StructuredWriter> createBigIntegerResolver(final BigInteger start) {
    ...
    private static Recycler<StringBuilder> createStringBuilderRecycler(final EventResolverContext context) {
        return context.getRecyclerFactory().create(StringBuilder::new, stringBuilder -> {
            final int maxLength = context.getStructuredWriter().getMaxStringLength();
    ...
    private static Consumer<StructuredWriter> createStringifiedLongResolver(
    ...
    private static Consumer<StructuredWriter> createStringifiedBigIntegerResolver(
            final BigInteger start, final Recycler<StringBuilder> stringBuilderRecycler) {
        final AtomicBigInteger counter = new AtomicBigInteger(start);
        return jsonWriter -> {
            final BigInteger number = counter.getAndIncrement();
    ...
    @Override
    public void resolve(final LogEvent ignored, final StructuredWriter jsonWriter) {

...
class CounterResolverTest {
    ...
    void no_arg_setup_should_start_from_zero() {
        final String eventTemplate = writeYaml(asMap("$resolver", "counter"));
        verify(eventTemplate, 0, 1);
    }

    @Test
    void positive_start_should_work() {
        final String eventTemplate = writeYaml(asMap("$resolver", "counter", "start", 3));
        verify(eventTemplate, 3, 4);
    }

    @Test
    void positive_start_should_work_when_stringified() {
        final String eventTemplate = writeYaml(asMap("$resolver", "counter", "start", 3, "stringified", true));
        verify(eventTemplate, "3", "4");
    }
    ...
    void min_long_should_work_when_overflow_enabled() {
        final String eventTemplate = writeYaml(asMap("$resolver", "counter", "start", Long.MIN_VALUE));
    ...
    void min_long_should_work_when_overflow_enabled_and_stringified() {
        final String eventTemplate =
                writeYaml(asMap("$resolver", "counter", "start", Long.MIN_VALUE, "stringified", true));
        verify(eventTemplate, "" + Long.MIN_VALUE, "" + (Long.MIN_VALUE + 1L));
    }

    @Test
    void max_long_should_work_when_overflowing() {
        final String eventTemplate = writeYaml(asMap("$resolver", "counter", "start", Long.MAX_VALUE));
    ...
    void max_long_should_work_when_overflowing_and_stringified() {
        final String eventTemplate =
                writeYaml(asMap("$resolver", "counter", "start", Long.MAX_VALUE, "stringified", true));
        verify(eventTemplate, "" + Long.MAX_VALUE, "" + Long.MIN_VALUE);
    }
    ...
    void max_long_should_work_when_not_overflowing() {
        final String eventTemplate =
                writeYaml(asMap("$resolver", "counter", "start", Long.MAX_VALUE, "overflowing", false));
    ...
    void max_long_should_work_when_not_overflowing_and_stringified() {
        final String eventTemplate = writeYaml(
    ...
    private static void verify(final String eventTemplate, final Object expectedNumber1, final Object expectedNumber2) {
        ...
        final Object deserializedLogEvent1 = JacksonFixture.getObjectMapper().readValue(serializedLogEvent1);
        ...
        final Object deserializedLogEvent2 = JacksonFixture.getObjectMapper().readValue(serializedLogEvent2);

...
class GelfLayoutTest {
    ...
    private static void test(final LogEvent logEvent) {
        ...
        verifyTimestamp(logEvent.getInstant(), yamlTemplateLayoutMap, gelfLayoutMap);
        Assertions.assertThat(yamlTemplateLayoutMap).isEqualTo(gelfLayoutMap);
    }
    ...
    private static void verifyTimestamp(
            ...
            final Map<String, Object> yamlTemplateLayoutMap,
            final Map<String, Object> gelfLayoutMap) {
        final Number yamlTemplateLayoutTimestamp = (Number) yamlTemplateLayoutMap.remove("timestamp");
        final Number gelfLayoutTimestamp = (Number) gelfLayoutMap.remove("timestamp");
        Assertions.assertThat(yamlTemplateLayoutTimestamp.doubleValue())
                .as(
                        "instantEpochSecs=%d.%d, yamlTemplateLayoutTimestamp=%s, gelfLayoutTimestamp=%s",
                        logEventInstant.getEpochSecond(),
                        logEventInstant.getNanoOfSecond(),
                        yamlTemplateLayoutTimestamp,
                        gelfLayoutTimestamp)

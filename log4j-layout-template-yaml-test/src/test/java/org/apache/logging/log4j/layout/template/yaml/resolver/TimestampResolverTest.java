...
import static org.apache.logging.log4j.layout.template.yaml.TestHelpers.uncheckedSleep;
import static org.apache.logging.log4j.layout.template.yaml.TestHelpers.withContextFromTemplate;
    ...
    void epoch_nanos_should_not_overlap() {
        ...
        withContextFromTemplate("TimestampResolverTest", eventTemplate, (loggerContext, appender) -> {
            ...
            final List<Long> logEvents = appender.getData().stream()
                    .map(yamlBytes -> {
                        final String yaml = new String(yamlBytes, StandardCharsets.UTF_8);
                        return (long) readYaml(yaml);
                    })
